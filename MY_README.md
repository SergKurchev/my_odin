...
## NBV Active Sensing (MODEL.NBV_ACTIVE = True)

При включении параметра `MODEL.NBV_ACTIVE = True` активируется расширение ODIN для **активного зрения**. В этом режиме модель обучается не только сегментировать объекты, но и понимать полноту видимости сцены, а также планировать следующее движение камеры.

### 1. Подготовка данных (Dataset Mapper)
В маппере `StrawberryDatasetMapper` происходят следующие изменения:
- **Расчет Coverage GT**: Система проверяет все доступные кадры в сцене. Если объект виден менее чем на 100 пикселей (`MIN_VISIBLE_PIXELS`) во всех кадрах чанка, он считается "скрытым".
- **coverage_gt**: Метка `1.0`, если есть скрытые объекты, и `0.0`, если все объекты сцены обнаружены и хорошо видны.
- **Camera Pose**: В батч передаются текущие координаты и ориентация камеры (`current_camera_position`, `current_camera_quaternion`).

### 2. Архитектура (Обертка NBVActiveODIN)
Базовая модель ODIN оборачивается в класс `NBVActiveODIN`, который добавляет две дополнительные головы (MLP):

*   **Coverage Head**: Предсказывает вероятность `p_hidden` (от 0 до 1), используя **Query Features** из декодера ODIN. Эти фичи содержат информацию о всех найденных объектах.
*   **NBV Head**: Предсказывает координаты (XYZ) и поворот (Quaternion) для следующей точки обзора. На вход получает pooled-фичи сцены и текущую позу камеры.

### 3. Функции потерь (Losses)
- **loss_coverage**: Binary Cross Entropy между предсказанием и `coverage_gt`.
- **loss_nbv (Self-Supervised)**: Состоит из двух частей:
    - **Explore Loss**: Наказывает за маленькие шаги, если `p_hidden` высок (модель получает "награду" за перемещение к новым точкам, если сцена не видна полностью).
    - **Reg Loss**: MSE-штраф, удерживающий длину шага около целевого значения (например, 0.3 метра), чтобы предсказания были стабильными.

### 4. Выходные данные (Outputs)
В режиме инференса модель возвращает стандартные маски сегментации, а также дополнительные поля:
- `p_hidden`: Оценка вероятности того, что в сцене еще остались невидимые объекты.
- `nbv_pos`: Рекомендуемые координаты XYZ для следующего снимка.
- `nbv_quat`: Рекомендуемая ориентация камеры (quaternion).

Это позволяет использовать ODIN в составе RL-пайплайна или простых алгоритмов обхода сцены для максимально быстрой и полной сегментации всех объектов.
