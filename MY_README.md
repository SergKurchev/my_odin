...
**Текущий рабочий коммит:** `f84cab4`
В этом состоянии гарантируется:
- Корректное совмещение облаков точек (исправлена инверсия Y в `backproject.py`).
- Правильный парсинг кватернионов Unity (формат `[x, y, z, w]`).
- Оптимизированная визуализация (один HTML-файл на сэмпл, без дублей, 200к точек для экономии VRAM).
- Настроенный `BestCheckpointer` (сохранение `model_best.pth`).
